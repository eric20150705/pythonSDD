...
- Git 分支名稱 MUST 與對應的 `specs/###-feature-name/` 目錄及規格文件中的
  `功能分支` 欄位完全一致，不得額外加入前綴或後綴。
- 功能分支 MUST 從預設整合分支建立，功能變更 MUST 透過合併請求合併，
  不得直接將功能提交推送至預設整合分支。
- 合併請求 MUST 連結或指向對應的 spec、plan 與 tasks 文件，並記錄適用的
  語法檢查、自動化測試與手動冒煙測試結果；未完成的品質門檻 MUST 明確列出。
- 合併請求確認合併成功後，來源分支 MUST 從遠端刪除，並同步從本地刪除；
...
**版本**：1.3.2｜**批准日期**：2026-08-26｜**最後修訂日期**：2026-08-27
